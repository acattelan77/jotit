(() => {
  ...
  const getImageExtension = (src, fallbackType = "") => {
    const dataMatch = /^data:(image\/[a-z0-9.+-]+);base64,/i.exec(src || "");
    const dataType = dataMatch?.[1]?.toLowerCase() || "";
    if (SUPPORTED_IMAGE_TYPES.has(dataType)) {
      return SUPPORTED_IMAGE_TYPES.get(dataType);
    }
    const normalizedFallbackType = String(fallbackType || "").toLowerCase();
    if (SUPPORTED_IMAGE_TYPES.has(normalizedFallbackType)) {
      return SUPPORTED_IMAGE_TYPES.get(normalizedFallbackType);
    }
    return getImageExtensionFromUrl(src) || "png";
  };

  const buildLibraryEntry = ({ data, existing = null, id, now = Date.now() }) => ({
    ...(existing || {}),
    id,
  ...
  const htmlToMarkdown = (html) => {
    ...
    const convert = (node) => {
      if (node.nodeType === Node.TEXT_NODE) {
        // U+200B (zero-width space) is a caret-positioning anchor the
        // editor inserts next to line breaks inside code blocks (see the
        // Enter-key handler in sidepanel.js) — an editing implementation
        // detail, never real note content, so it never belongs in
        // exported/saved text.
  ...
  const markdownToHtml = (markdown) => {
    ...
    const blockquoteStash = [];
    const stashBq = (fragment) => {
      const token = `\u0000BQ${blockquoteStash.length}\u0000`;
      blockquoteStash.push(fragment);
      return token;
    };

    html = html.replace(/^> (.+)$/gm, (_, text) =>
      stashBq(`<blockquote>${text}</blockquote>`)
    );

    html = html.replace(/>/g, "&gt;");

    // Fenced/inline code content must not be re-scanned by the passes
    // below (e.g. `**kwargs` or `a == b` inside real code would otherwise
    // get spuriously bolded/highlighted) — stash it behind an opaque token
    // and restore it verbatim once every other pass has run.
    const codeStash = [];
    const stashCode = (fragment) => {
      const token = `\u0000CODE${codeStash.length}\u0000`;
      codeStash.push(fragment);
      return token;
    };

    html = html.replace(/```([\s\S]*?)```/g, (_, code) =>
      stashCode(`<pre><code>${code.trim()}</code></pre>`)
    );
    ...
    const wrapListItems = (listType, tag) => {
      const item = `<li data-list="${listType}">[^\\n]*<\\/li>`;
      const list = new RegExp(`${item}(?:\\n+${item})*`, "g");
      html = html.replace(
        list,
        (match) => `<${tag}>${match.replace(/\n+/g, "")}</${tag}>`
      );
    };
    // Newlines delimit Markdown list items; they must not survive inside
    // the reconstructed list as <br> elements when the general newline
    // conversion below runs. Accept one-or-more separators here so notes
    // already affected by the old round-trip bug heal when reopened.
    wrapListItems("ol", "ol");
    wrapListItems("ul", "ul");
    ...
    html = html.replace(/\[([^\]]+)\]\(([^)]+)\)/g, (match, text, href) => {
      ...
      const title = escapeAttribute(`${safeHref} — ⌘/Ctrl+Click to open`);
    ...
    html = html.replace(/\u0000BQ(\d+)\u0000/g, (_, i) => blockquoteStash[Number(i)]);
